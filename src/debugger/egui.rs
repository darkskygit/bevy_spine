use egui::*;
use egui_extras::*;
use rusty_spine::{AnimationState, Attachment, SlotHandle};

use crate::{BoneHandle, Skeleton};
...
pub fn egui_spine_debugger(
    ...
    let mut bone_windows = ctx.data(|ctx| {
        ctx.get_temp::<Vec<BoneHandle>>(Id::new(format!("{}-bones", unique.clone())))
            .unwrap_or(vec![])
    });
    Window::new(title)
        ...
        .show(ctx, |ui| {
            ...
            let table = TableBuilder::new(ui)
                ...
                .column(Column::initial(110.0).at_least(110.0))
    ...
    ctx.data_mut(|ctx| ctx.insert_temp(Id::new(format!("{}-bones", unique)), bone_windows));
}
